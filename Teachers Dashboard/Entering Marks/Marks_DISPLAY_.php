<?php
     ...
     if(isset($_POST['submit']))
     {
        ...
        $class= $_POST['class'];
        if ($sub_code=="" || $semester=="" || $class=="")
        {
          echo " <script>alert('Please enter both the details before Proceeding !!');</script>";
        }
        $query = "SELECT * from results where sub_code='$sub_code' and semester='$semester' and class='$class';";
        ...
        if(mysqli_num_rows($result)>0)
        {
            ...
            header("location:http://localhost/project/Front_End/dashboard/Teachers%20Dashboard/Entering%20Marks/Marks_entry_DISPLAY_DISPLAY.php");
            exit();
        }
        else
        {
          echo " <script>alert('There is no such combination in the Results Table!!');</script>";
        }
      }
  ...
  <div class="row">
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2"><strong><br>Results Display</strong></h1>
      </div>
      <form method="post">
